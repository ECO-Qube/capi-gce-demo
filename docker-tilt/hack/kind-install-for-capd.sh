...
# Downloaded and adapted from: https://github.com/kubernetes-sigs/cluster-api/blob/main/hack/kind-install-for-capd.sh
...
KIND_CLUSTER_VERSION=${KIND_CLUSTER_VERSION:-"v1.27.3"}
...
cat <<EOF | kind create cluster --name="$KIND_CLUSTER_NAME" --image="kindest/node:$KIND_CLUSTER_VERSION" --config=-
kind: Cluster
